...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            time.sleep(1)
# ============================================================
# 🔥 NEW DASHBOARD FEATURES – User Mgmt, Subscriptions, Channels, Broadcast, Editor, Stats JSON
# ============================================================
...
# ---- 1. User Detail & Management (Ban/Unban/Set Limit) ----
@app.route('/dashboard/admin/user/<int:uid>')
def user_detail(uid):
    if not session.get('is_admin', False):
        abort(403)
    files = user_files.get(uid, [])
    running = sum(1 for f, _ in files if is_bot_running(uid, f))
    status = get_user_status(uid)
    expiry = user_subscriptions.get(uid, {}).get('expiry')
    expiry_str = format_datetime(expiry) if expiry else "None"
    limit = get_user_file_limit(uid)
    limit_str = "Unlimited" if limit == float('inf') else str(limit)
    html = f"""
    <h2>👤 User {uid}</h2>
    <ul>
        <li><strong>Status:</strong> {status}</li>
        <li><strong>Files:</strong> {len(files)} / {limit_str}</li>
        <li><strong>Running Scripts:</strong> {running}</li>
        <li><strong>Subscription Expiry:</strong> {expiry_str}</li>
    </ul>
    <form method="POST" action="/dashboard/admin/user/{uid}/ban">
        <input type="hidden" name="reason" value="Banned via dashboard">
        <button type="submit">🚫 Ban User</button>
    </form>
    <form method="POST" action="/dashboard/admin/user/{uid}/unban">
        <button type="submit">✅ Unban User</button>
    </form>
    <form method="POST" action="/dashboard/admin/user/{uid}/setlimit">
        <input type="number" name="limit" placeholder="New file limit" required>
        <button type="submit">Set Limit</button>
    </form>
    <form method="POST" action="/dashboard/admin/user/{uid}/removelimit">
        <button type="submit">Remove Limit</button>
    </form>
    <br><a href="{url_for('admin_users')}">Back to Users</a>
    """
    return html

@app.route('/dashboard/admin/user/<int:uid>/ban', methods=['POST'])
def ban_user_dashboard(uid):
    if not session.get('is_admin', False):
        abort(403)
    reason = request.form.get('reason', 'Banned via dashboard')
    if ban_user_db(uid, reason, session['user_id']):
        ...
        return "Ban failed", 500

@app.route('/dashboard/admin/user/<int:uid>/unban', methods=['POST'])
def unban_user_dashboard(uid):
    if not session.get('is_admin', False):
        abort(403)
    if unban_user_db(uid):
        return redirect(url_for('user_detail', uid=uid))
    else:
        return "Unban failed", 500

@app.route('/dashboard/admin/user/<int:uid>/setlimit', methods=['POST'])
def set_user_limit_dashboard(uid):
    if not session.get('is_admin', False):
        abort(403)
    try:
        limit = int(request.form['limit'])
        if set_user_limit_db(uid, limit, session['user_id']):
            return redirect(url_for('user_detail', uid=uid))
        else:
            return "Failed to set limit", 500
    except:
        return "Invalid limit", 400

@app.route('/dashboard/admin/user/<int:uid>/removelimit', methods=['POST'])
def remove_user_limit_dashboard(uid):
    if not session.get('is_admin', False):
        abort(403)
    if remove_user_limit_db(uid):
        return redirect(url_for('user_detail', uid=uid))
    else:
        return "Failed to remove limit", 500

# ---- 2. Subscription Management ----
@app.route('/dashboard/admin/subscription/<int:uid>', methods=['GET', 'POST'])
def manage_subscription(uid):
    if not session.get('is_admin', False):
        abort(403)
    msg = ""
    if request.method == 'POST':
        ...
        if action == 'add':
            ...
            msg = f"✅ Added {days} days. New expiry: {new_expiry.strftime('%Y-%m-%d')}"
        elif action == 'remove':
            ...
            if expiry and expiry > datetime.now():
                days_left = (expiry - datetime.now()).days
                msg = f"✅ Active – {days_left} days left (expires {expiry.strftime('%Y-%m-%d')})"
            else:
                msg = "❌ No active subscription."
    expiry = user_subscriptions.get(uid, {}).get('expiry')
    expiry_str = format_datetime(expiry) if expiry else "None"
    html = f"""
    <h2>💳 Subscription for User {uid}</h2>
    <p><strong>Current expiry:</strong> {expiry_str}</p>
    <form method="POST">
        <input type="hidden" name="action" value="add">
        <label>Add days: <input type="number" name="days" value="30" min="1"></label>
        <button type="submit">➕ Add / Extend</button>
    </form>
    <form method="POST">
...
# ---- 3. Mandatory Channels Management ----
@app.route('/dashboard/admin/channels', methods=['GET', 'POST'])
def manage_channels():
    if not session.get('is_admin', False):
        abort(403)
    msg = ""
    if request.method == 'POST':
        ...
        if action == 'add':
            ...
            try:
                ...
                cid = str(chat.id)
                cname = chat.title
                cusername = f"@{chat.username}" if chat.username else ""
                if save_mandatory_channel(cid, cusername, cname, session['user_id']):
                    msg = f"✅ Channel '{cname}' added."
                else:
    ...
    html = "<h2>📢 Mandatory Channels</h2>"
    if mandatory_channels:
        html += "<ul>"
        for cid, info in mandatory_channels.items():
            html += f"<li>{info.get('name', 'Unknown')} ({cid}) – {info.get('username', 'No username')}</li>"
        html += "</ul>"
    else:
        html += "<p>No mandatory channels set.</p>"
    html += f"""
    <h3>Add Channel</h3>
    <form method="POST">
        <input type="hidden" name="action" value="add">
        <label>Channel ID or @username: <input type="text" name="channel_id" required></label>
        <button type="submit">Add</button>
    </form>
    <h3>Remove Channel</h3>
    <form method="POST">
        <input type="hidden" name="action" value="remove">
        <label>Channel ID: <input type="text" name="channel_id" required></label>
        <button type="submit">Remove</button>
    </form>
    <p>{msg}</p>
    <br><a href="{url_for('dashboard')}">Back</a>
    """
    return html

# ---- 4. Broadcast (Text/Photo/Video) ----
@app.route('/dashboard/admin/broadcast', methods=['GET', 'POST'])
def admin_broadcast():
    if not session.get('is_admin', False):
        abort(403)
    msg = ""
    if request.method == 'POST':
        ...
        if not text and not photo and not video:
            msg = "❌ Send text or upload a photo/video."
        else:
            def broadcast_job():
                sent = 0
                for uid in list(active_users):
                    try:
                        ...
                        pass
                bot.send_message(session['user_id'], f"✅ Broadcast sent to {sent} users.")
            threading.Thread(target=broadcast_job).start()
            msg = "🚀 Broadcast started. You'll receive a confirmation when done."
    html = f"""
    <h2>📢 Broadcast Message</h2>
    <form method="POST" enctype="multipart/form-data">
        <label>Text message (optional):</label><br>
        <textarea name="text" rows="4" cols="50"></textarea><br><br>
        <label>Photo (optional): <input type="file" name="photo" accept="image/*"></label><br>
        <label>Video (optional): <input type="file" name="video" accept="video/*"></label><br>
        <label>Caption for media: <input type="text" name="caption"></label><br><br>
...
# ---- 5. Real‑time Stats (JSON) ----
@app.route('/dashboard/admin/stats/json')
def stats_json():
    if not session.get('is_admin', False):
        abort(403)
...
# ---- 6. Script Editor (View & Edit) ----
@app.route('/dashboard/edit/<int:uid>/<file>', methods=['GET', 'POST'])
def edit_script(uid, file):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    if session['user_id'] != uid and not session.get('is_admin', False):
        abort(403)
    ...
    if request.method == 'POST':
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content)
        return redirect(url_for('edit_script', uid=uid, file=file))
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read()
    html = f"""
    <h2>Editing: {file} (User {uid})</h2>
    <form method="POST">
        <textarea name="content" rows="30" cols="100" style="font-family:monospace;">{content}</textarea><br>
