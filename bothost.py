...
def parse_duration(duration_str):
    """
    Parse duration string like '30d', '2h', '90m', '1w', '6m', '1y',
    'lifetime', 'forever', '∞', '0' → returns timedelta or None for lifetime.
    """
    ...
    if match:
        ...
        elif unit == 'y':   # year (365 days)
            return timedelta(days=num*365)

    # If it's a plain number, treat as days (backward compatibility)
    if duration_str.isdigit():
        return timedelta(days=int(duration_str))

    raise ValueError("Invalid duration format. Use like '30d', '2h', '90m', '1w', '6m', '1y', or 'lifetime'.")
...
TOKEN = os.getenv('TOKEN', '8666591401:AAG-vo46rho09uh_cdT8vYpPSpaVyBf_0sQ')
OWNER_ID = int(os.getenv('OWNER_ID', 8962068652 ))
ADMIN_ID = int(os.getenv('ADMIN_ID', 8013610670))
YOUR_USERNAME = os.getenv('YOUR_USERNAME', 'superiorx0x0')
...
SUBSCRIBED_USER_LIMIT = int(os.getenv('SUBSCRIBED_USER_LIMIT', 2))
...
# ============================================
# 🔥 DATABASE FIX - FORCE FIX PERMISSIONS
# ============================================
import sqlite3

def fix_database_issue():
    """Fix database permissions and corruption issues"""
    print("="*60)
    ...
    try:
        ...
        if not os.path.exists(IROTECH_DIR):
            os.makedirs(IROTECH_DIR, exist_ok=True)
            print(f"✅ Created directory: {IROTECH_DIR}")
        ...
        # 3. If database exists, check if it's valid
        if os.path.exists(DATABASE_PATH):
            try:
                ...
                print(f"⚠️ Database corrupted or locked: {e}")
                print("🔄 Backing up and recreating...")
                
                # Backup corrupted database
                backup_path = DATABASE_PATH + '.corrupted_backup'
                try:
                    if os.path.exists(DATABASE_PATH):
                ...
                try:
                    ...
                    if os.path.exists(DATABASE_PATH + '-journal'):
                        os.remove(DATABASE_PATH + '-journal')
                    ...
                    print("✅ Removed corrupted database files")
                except Exception as e2:
                    print(f"⚠️ Could not remove corrupted files: {e2}")
        ...
        print("💡 Please run: sudo chmod -R 777 " + IROTECH_DIR)
        sys.exit(1)
...
# --- Database Setup ---
def init_db():
    """Initialize the database with required tables - FIXED VERSION"""
    logger.info(f"Initializing database at: {DATABASE_PATH}")
    
    # Ensure directory exists before DB operations
    try:
    ...
    conn = None
    try:
        # Use timeout to avoid locking issues
        ...
        # --- PERMANENT HOSTING TABLES ---
        c.execute('''CREATE TABLE IF NOT EXISTS bot_hosts
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS free_host_logs
        ...
        if "unable to open database file" in str(e):
            logger.error("💡 FIX: Run: sudo chmod -R 777 " + IROTECH_DIR)
    except Exception as e:
...
# ============================================================
# 🆕 PERMANENT HOSTING DATABASE FUNCTIONS
# ============================================================

def save_bot_host(user_id, file_name, file_type):
    """Save bot host record to database (permanent storage)"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            ...
            c.execute('''INSERT OR REPLACE INTO bot_hosts 
                         (user_id, file_name, file_type, start_time, last_active, status)
                         VALUES (?, ?, ?, ?, ?, ?)''',
                      (user_id, file_name, file_type, now, now, 'running'))
            conn.commit()
            logger.info(f"Saved bot host: user={user_id}, file={file_name}")
            return True
        except sqlite3.Error as e:
            logger.error(f"❌ SQLite error saving bot host: {e}")
            return False
        except Exception as e:
            logger.error(f"❌ Unexpected error saving bot host: {e}", exc_info=True)
            return False
        finally:
            conn.close()

def update_bot_host_status(user_id, file_name, status):
    """Update bot host status in database"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('''UPDATE bot_hosts 
                         SET status = ?, last_active = ?
                         WHERE user_id = ? AND file_name = ?''',
                      (status, datetime.now().isoformat(), user_id, file_name))
            conn.commit()
            logger.info(f"Updated bot host: user={user_id}, file={file_name}, status={status}")
            return True
        except sqlite3.Error as e:
            logger.error(f"❌ SQLite error updating bot host: {e}")
            return False
        except Exception as e:
            logger.error(f"❌ Unexpected error updating bot host: {e}", exc_info=True)
            return False
        finally:
            conn.close()

def delete_bot_host(user_id, file_name):
    """Delete bot host record from database"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('DELETE FROM bot_hosts WHERE user_id = ? AND file_name = ?',
                      (user_id, file_name))
            conn.commit()
            logger.info(f"Deleted bot host: user={user_id}, file={file_name}")
            return True
        except sqlite3.Error as e:
            logger.error(f"❌ SQLite error deleting bot host: {e}")
            return False
        except Exception as e:
            logger.error(f"❌ Unexpected error deleting bot host: {e}", exc_info=True)
            return False
        finally:
            conn.close()

def get_bot_hosts(user_id=None):
    """Get bot hosts from database"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            if user_id is not None:
                c.execute('''SELECT user_id, file_name, file_type, start_time, last_active, status
                             FROM bot_hosts WHERE user_id = ?''', (user_id,))
            else:
                c.execute('''SELECT user_id, file_name, file_type, start_time, last_active, status
                             FROM bot_hosts''')
            rows = c.fetchall()
            ...
            logger.error(f"❌ SQLite error getting bot hosts: {e}")
            return []
        except Exception as e:
            logger.error(f"❌ Unexpected error getting bot hosts: {e}", exc_info=True)
            return []

def save_free_host_log(user_id, file_name, reason, notified=False):
    """Save free host stop log"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            now = datetime.now().isoformat()
            c.execute('''INSERT INTO free_host_logs (user_id, file_name, stopped_at, reason, notified)
                         VALUES (?, ?, ?, ?, ?)''',
                      (user_id, file_name, now, reason, 1 if notified else 0))
            conn.commit()
            logger.info(f"Saved free host log: user={user_id}, file={file_name}, reason={reason}")
            return True
        except sqlite3.Error as e:
            logger.error(f"❌ SQLite error saving free host log: {e}")
            return False
        except Exception as e:
            logger.error(f"❌ Unexpected error saving free host log: {e}", exc_info=True)
            return False
        finally:
            conn.close()

def get_free_host_logs(user_id=None, limit=50):
    """Get free host logs"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            if user_id is not None:
                c.execute('''SELECT log_id, user_id, file_name, stopped_at, reason, notified
                             FROM free_host_logs WHERE user_id = ?
                             ORDER BY stopped_at DESC LIMIT ?''', (user_id, limit))
            else:
                c.execute('''SELECT log_id, user_id, file_name, stopped_at, reason, notified
                             FROM free_host_logs ORDER BY stopped_at DESC LIMIT ?''', (limit,))
            rows = c.fetchall()
            conn.close()
            return rows
        except sqlite3.Error as e:
            logger.error(f"❌ SQLite error getting free host logs: {e}")
            return []
        except Exception as e:
            logger.error(f"❌ Unexpected error getting free host logs: {e}", exc_info=True)
            return []

# ============================================================
# 🆕 FREE USER BOT EXPIRATION SYSTEM
# ============================================================

FREE_USER_BOT_DURATION = timedelta(hours=24)  # Free users get 24 hours
...
def check_free_bot_expiration():
    """Check and stop expired free user bots"""
    logger.info("🔍 Checking free user bot expiration...")
    
    # Get all bot hosts from database
    hosts = get_bot_hosts()
    
    stopped_count = 0
    for host in hosts:
        user_id, file_name, file_type, start_time_str, last_active_str, status = host
        ...
        # Check if bot has been running for more than 24 hours
        try:
            start_time = datetime.fromisoformat(start_time_str)
            elapsed = datetime.now() - start_time
            
            if elapsed >= FREE_USER_BOT_DURATION:
                logger.info(f"⏰ Free user bot expired: user={user_id}, file={file_name}, elapsed={elapsed}")
                ...
                update_bot_host_status(user_id, file_name, 'expired')
                
                # Save log
                save_free_host_log(user_id, file_name, f"Free user 24-hour limit reached (elapsed: {elapsed})", notified=False)
                
                # Send notification to user
                try:
                    notification = (
                        f"⏰ **Bot Hosting Time Limit Reached!**\n\n"
                        f"📁 **Bot:** `{file_name}`\n"
                        f"⏱️ **Uptime:** {str(elapsed).split('.')[0]}\n"
                        f"📌 **Status:** ❌ Stopped (Free User Limit)\n\n"
                        f"🔄 **To restart your bot:**\n"
                        f"1. Go to **📂 Check Files**\n"
                        f"2. Select your bot\n"
                        f"3. Click **🟢 Start**\n\n"
                        f"⭐ **Upgrade to Premium** for unlimited uptime!\n"
                        f"Contact @{YOUR_USERNAME.replace('@', '')} for details."
                    )
                    ...
                    with DB_LOCK:
                        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
                        c = conn.cursor()
                        c.execute('''UPDATE free_host_logs SET notified = 1 
                                     WHERE user_id = ? AND file_name = ? AND notified = 0''',
                                  (user_id, file_name))
                        conn.commit()
                        conn.close()
                    ...
                    logger.error(f"Failed to notify user {user_id} about bot expiration: {e}")
                    
        except Exception as e:
            logger.error(f"Error checking expiration for user={user_id}, file={file_name}: {e}")
    
    if stopped_count > 0:
        logger.info(f"✅ Stopped {stopped_count} expired free user bots")
    else:
        logger.info("✅ No expired free user bots found")

def start_free_bot_monitor():
    """Start a background thread to monitor free user bot expiration"""
    def monitor_loop():
        while True:
            try:
                check_free_bot_expiration()
            except Exception as e:
                logger.error(f"Error in free bot monitor: {e}", exc_info=True)
            # Check every hour
            time.sleep(3600)
    
    thread = threading.Thread(target=monitor_loop, daemon=True)
    thread.start()
    logger.info("🔄 Free bot expiration monitor started (checks every hour)")

# ============================================================
# 🆕 PERMANENT HOSTING - RESTORE BOTS ON RESTART
# ============================================================

def restore_bots_on_startup():
    """Restore running bots from database on startup"""
    logger.info("🔄 Restoring bots from database...")
    
    hosts = get_bot_hosts()
    restored_count = 0
    
    for host in hosts:
        user_id, file_name, file_type, start_time, last_active, status = host
        
        # Only restore if status is 'running'
        if status != 'running':
            continue
        ...
        if not os.path.exists(file_path):
            logger.warning(f"File not found for user {user_id}: {file_name}")
            update_bot_host_status(user_id, file_name, 'missing_file')
            continue
        
        # Check if already running
        if is_bot_running(user_id, file_name):
            logger.info(f"Bot already running: user={user_id}, file={file_name}")
            continue
        
        # Restore the bot
        logger.info(f"Restoring bot: user={user_id}, file={file_name}, type={file_type}")
        
        try:
            # Create a dummy message object for feedback (only logs)
            class DummyMessage:
            ...
            if file_type == 'py':
                threading.Thread(target=run_script, args=(file_path, user_id, user_folder, file_name, dummy_msg)).start()
            elif file_type == 'js':
                threading.Thread(target=run_js_script, args=(file_path, user_id, user_folder, file_name, dummy_msg)).start()
            else:
                continue
            ...
            logger.error(f"Failed to restore bot {file_name} for user {user_id}: {e}")
            update_bot_host_status(user_id, file_name, 'restore_failed')
    
    logger.info(f"✅ Restored {restored_count} bots from database")
...
def send_host_notification(user_id, file_name, file_type, is_safe=True, security_msg=""):
    """Send rich notification to admins when a user hosts a new bot"""
    try:
        # Get user info
        ...
        host_type = "⭐ Premium" if is_premium_user(user_id) else "🆓 Free (24h limit)"
        ...
        if not is_premium_user(user_id):
            notification += "\n⏰ **Free user:** Bot will stop after 24 hours."
        else:
            notification += "\n🌟 **Premium user:** Bot will run permanently (until stopped)."
        
        notification += "\n📌 Bot is now live and running."
        
        # Send to all admins and owner
        ...
        logger.error(f"Error sending host notification: {e}", exc_info=True)
        ...
        try:
            ...
            if not is_running:
                ...
                if script_key in bot_scripts:
                    del bot_scripts[script_key]
                # Update database status
                update_bot_host_status(script_owner_id, file_name, 'stopped')
            return is_running
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('🔑 Manage Keys', callback_data='manage_keys')  # NEW
        ]
        ...
        markup.add(admin_buttons[10]) # Manage Keys
        ...
        markup.add(types.InlineKeyboardButton('🎫 Redeem Key', callback_data='redeem_key'))  # NEW
        markup.add(buttons[5]) # Contact
    return markup
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        # Security check for ZIP – now just logs warnings, no approval
        is_safe, security_msg = scan_zip_security(zip_path)
        if not is_safe:
            logger.warning(f"⚠️ Unsafe ZIP from {user_id}: {security_msg}")
            # Send warning to admins but still proceed
            send_host_notification(user_id, file_name_zip, "ZIP", is_safe=False, security_msg=security_msg)

        # Process ZIP file (auto-start)
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        try:
            ...
            save_bot_host(script_owner_id, file_name, 'py')
            
            bot.reply_to(message_obj_for_reply, f"✅ Python script '{file_name}' started! (PID: {process.pid}) (For User: {script_owner_id})")
            
            # Send notification about hosting duration
            if not is_premium_user(script_owner_id):
                bot.reply_to(message_obj_for_reply, 
                             f"⏰ **Free User Notice:** Your bot will run for **24 hours**.\n"
                             f"After that, it will automatically stop.\n"
                             f"⭐ Upgrade to Premium for permanent hosting!")
        except FileNotFoundError:
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        try:
            ...
            # Save to permanent database
            save_bot_host(script_owner_id, file_name, 'js')
...
def _logic_send_welcome(message):
    ...
    # Check if user has any expired free bots
    expired_bots = []
    hosts = get_bot_hosts(user_id)
    for host in hosts:
        if host[5] == 'expired':
            expired_bots.append(host[1])
    
    expired_notice = ""
    if expired_bots:
        expired_notice = f"\n⏰ **Expired Bots:** {', '.join(f'`{b}`' for b in expired_bots)}\n"
...
**⏰ Hosting Duration:**
• **Free Users:** Bot runs for 24 hours, then auto-stops
• **Premium Users:** Bot runs permanently (until manually stopped)
...
def generate_key(limit_value, duration=None, created_by=OWNER_ID):
    """
    Generate a rich key with given limit and duration.
    duration: None → lifetime, or a timedelta object.
    """
    # Generate unique key (format: PREMIUM-XXXXXX)
    import random
    while True:
        ...
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('SELECT 1 FROM keys WHERE key_code = ?', (key_code,))
        if not c.fetchone():
            conn.close()
            break
        conn.close()

    expiry = None if duration is None else (datetime.now() + duration).isoformat()
    created_at = datetime.now().isoformat()
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('''INSERT INTO keys (key_code, limit_value, expiry, created_at, created_by)
                         VALUES (?, ?, ?, ?, ?)''',
                      (key_code, limit_value, expiry, created_at, created_by))
            conn.commit()
            logger.info(f"Generated key {key_code} with limit {limit_value}, expiry {expiry or 'Lifetime'}")
            return key_code
        except Exception as e:
...
def redeem_key(user_id, key_code):
    with DB_LOCK:
        ...
        c = conn.cursor()
        try:
            c.execute('''SELECT key_id, limit_value, expiry, used, used_by
                         FROM keys WHERE key_code = ?''', (key_code,))
            row = c.fetchone()
            if not row:
                return False, "❌ Key not found."
            key_id, limit_value, expiry_str, used, used_by = row
            if used:
            ...
            # if expiry_str is None, it's lifetime – no expiry check
            if set_user_limit_db(user_id, limit_value, user_id):
                c.execute('UPDATE keys SET used = 1, used_by = ? WHERE key_id = ?',
                          (user_id, key_id))
                conn.commit()
                logger.info(f"User {user_id} redeemed key {key_code} for limit {limit_value}")
                return True, f"✅ Success! Your file limit is now **{limit_value}**."
            else:
                return False, "❌ Failed to set limit. Contact admin."
        except Exception as e:
...
def list_keys(limit=50):
    """Return list of keys (last 50) with details."""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('''SELECT key_id, key_code, limit_value, expiry, used, used_by, created_at
                     FROM keys ORDER BY key_id DESC LIMIT ?''', (limit,))
        rows = c.fetchall()
        conn.close()
        return rows
...
def _logic_statistics(message):
    ...
    # Get hosted bots from database
    total_hosted = len(get_bot_hosts())
    expired_hosts = len([h for h in get_bot_hosts() if h[5] == 'expired'])
    
    # Get free host logs
    free_logs = len(get_free_host_logs())
    ...
    stats_msg_base = (f"📊 Bot Statistics:\n\n"
                      ...
                      f"📦 Total Hosted Bots: {total_hosted}\n"
                      f"⏰ Expired Free Bots: {expired_hosts}\n"
                      f"📋 Free Host Logs: {free_logs}\n")
...
def command_genkey(message):
    ...
    try:
        ...
        if len(parts) >= 3:
            ...
            # Default to 30 days if no duration given
        ...
        logger.error(f"Error in genkey: {e}", exc_info=True)
        bot.reply_to(message, f"❌ Error: {str(e)}")
...
def handle_file_upload_doc(message):
    ...
    try:
        ...
        if file_ext == '.zip':
            # handle_zip_file now auto-starts and sends notification internally
            ...
            # Security check – logs warnings but always proceeds
            is_safe, security_msg = check_code_security(file_path, file_ext[1:])
            if not is_safe:
                logger.warning(f"⚠️ Unsafe script from {user_id}: {file_name} - {security_msg}")

            # Send rich notification to all admins/owner (for both safe & unsafe)
...
def handle_callbacks(call):
    ...
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats',
                                                                 'check_subscription_status', 'manual_install']:
...
def file_control_callback(call):
    try:
        ...
        # Get hosting info
        hosts = get_bot_hosts(script_owner_id)
        host_info = next((h for h in hosts if h[1] == file_name), None)
        host_status = ""
        if host_info:
            status = host_info[5]
            if status == 'running':
                host_status = "🌟 Permanent" if is_premium_user(script_owner_id) else "⏰ 24h Limit"
            elif status == 'expired':
                host_status = "⏰ Expired (Free Limit)"
            elif status == 'stopped':
                host_status = "🛑 Stopped"
            else:
                host_status = f"📌 {status}"
        else:
            host_status = "📌 Not hosted"
        
        try:
            bot.edit_message_text(
                f"⚙️ Controls for: `{file_name}` ({file_type}) of User `{script_owner_id}`\n"
                f"Status: {status_text}\n"
                f"Hosting: {host_status}",
...
def delete_bot_callback(call):
    try:
        ...
        remove_user_file_db(script_owner_id, file_name)
        # Delete from bot_hosts
        delete_bot_host(script_owner_id, file_name)
        ...
        if ban_user_db(user_id, reason, admin_id):
            ...
            for file_name, _ in user_files.get(user_id, []):
                ...
                if script_key in bot_scripts:
                    ...
                    del bot_scripts[script_key]
                # Update database
                update_bot_host_status(user_id, file_name, 'banned')
...
def process_user_info(message):
    ...
    try:
        ...
        hosts = get_bot_hosts(user_id)
        if hosts:
            info_parts.append("📦 **Hosted Bots:**")
            for host in hosts:
                h_file, h_type, h_start, h_last, h_status = host[1], host[2], host[3], host[4], host[5]
                status_icon = "🟢" if h_status == 'running' else "🔴" if h_status == 'stopped' else "⏰" if h_status == 'expired' else "❓"
                info_parts.append(f"  {status_icon} `{h_file}` ({h_type}) - {h_status}")
...
def display_users_list(chat_id, message_id, users_list, page, total_pages, chunk_size):
    ...
    for i, user_id in enumerate(user_chunk, start=start_idx + 1):
        ...
        # Check if user has hosted bots
        hosts = get_bot_hosts(user_id)
        host_count = len(hosts)
        host_indicator = f" [{host_count} hosted]" if host_count > 0 else ""
        
        message_text += f"{i}. `{user_id}` {status}{host_indicator}\n"
...
def system_info_callback(call):
    ...
    try:
        ...
        info_parts.append(f"• Hosted Bots: {len(get_bot_hosts())}")
        info_parts.append(f"• Expired Bots: {len([h for h in get_bot_hosts() if h[5] == 'expired'])}")
...
def bot_performance_callback(call):
    ...
    try:
        ...
        performance_parts.append(f"• Hosted Bots: {total_hosted}")
...
if __name__ == '__main__':
    ...
    # Start free bot expiration monitor
    start_free_bot_monitor()
    ...
    logger.info("="*50 + "\n🤖 DIV Hosting Bot Starting Up...\n" + f"🐍 Python: {sys.version.split()[0]}\n" +
                ...
                f"📦 Hosted Bots: {len(get_bot_hosts())}\n" + "="*50)
    keep_alive()
